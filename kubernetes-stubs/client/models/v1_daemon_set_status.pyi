# Code generated by `stubgen`. DO NOT EDIT.
from kubernetes.client.configuration import Configuration as Configuration
from typing import Any

class V1DaemonSetStatus:
    openapi_types: Any
    attribute_map: Any
    local_vars_configuration: Any
    discriminator: Any
    def __init__(self, collision_count: Any | None = ..., conditions: Any | None = ..., current_number_scheduled: Any | None = ..., desired_number_scheduled: Any | None = ..., number_available: Any | None = ..., number_misscheduled: Any | None = ..., number_ready: Any | None = ..., number_unavailable: Any | None = ..., observed_generation: Any | None = ..., updated_number_scheduled: Any | None = ..., local_vars_configuration: Any | None = ...) -> None: ...
    @property
    def collision_count(self): ...
    @collision_count.setter
    def collision_count(self, collision_count) -> None: ...
    @property
    def conditions(self): ...
    @conditions.setter
    def conditions(self, conditions) -> None: ...
    @property
    def current_number_scheduled(self): ...
    @current_number_scheduled.setter
    def current_number_scheduled(self, current_number_scheduled) -> None: ...
    @property
    def desired_number_scheduled(self): ...
    @desired_number_scheduled.setter
    def desired_number_scheduled(self, desired_number_scheduled) -> None: ...
    @property
    def number_available(self): ...
    @number_available.setter
    def number_available(self, number_available) -> None: ...
    @property
    def number_misscheduled(self): ...
    @number_misscheduled.setter
    def number_misscheduled(self, number_misscheduled) -> None: ...
    @property
    def number_ready(self): ...
    @number_ready.setter
    def number_ready(self, number_ready) -> None: ...
    @property
    def number_unavailable(self): ...
    @number_unavailable.setter
    def number_unavailable(self, number_unavailable) -> None: ...
    @property
    def observed_generation(self): ...
    @observed_generation.setter
    def observed_generation(self, observed_generation) -> None: ...
    @property
    def updated_number_scheduled(self): ...
    @updated_number_scheduled.setter
    def updated_number_scheduled(self, updated_number_scheduled) -> None: ...
    def to_dict(self): ...
    def to_str(self): ...
    def __eq__(self, other): ...
    def __ne__(self, other): ...
